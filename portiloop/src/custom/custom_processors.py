import numpy as np
from scipy.signal import firwin, butter, iirnotch
...
NOTCH_BANDWIDTH_HZ = 0.5088669900923876


class Notch:
    def __init__(self, power_line_fq, nb_channels, fs=250):
        if fs == 250:
            # use pre-computed values from the Portiloop PLOS ONE paper
            if power_line_fq == 60:
            ...
            # FIXME: This was generated by Claude Opus 5.5, double-check that the math works
            b, a = iirnotch(power_line_fq, Q=power_line_fq / NOTCH_BANDWIDTH_HZ, fs=fs)
            self.notch_coeff1, self.notch_coeff2 = a[1], a[2]
            self.notch_coeff3, self.notch_coeff4, self.notch_coeff5 = b
...
class SpindleFilter(Filter):
    def __init__(self, config_dict, lsl_streamer, csv_recorder):
        ...
        else:  # Note: this assumes 250Hz sampling frequency (as does the spindle detection model)
            fir_coef = [
        ...
        if self.use_notch:
            self.filter_parts.append(Notch(power_line_fq, nb_channels, sampling_rate))
        if self.use_std:
...
class SlowOscillationFilter(Filter):
    def __init__(self, config_dict, lsl_streamer=None, csv_recorder=None):
        ...
        notch = Notch(power_line_fq, nb_channels, sampling_rate)
