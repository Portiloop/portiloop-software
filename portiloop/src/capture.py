...
from portiloop.src.utils import (
    ADSFrontend,
    Dummy,
    FileFrontend,
    LSLStreamer,
...
def start_capture(
    ...
    lsl_streamer = (
        ...
        if capture_dictionary["lsl"]
        else Dummy()
    )
    stimulator = stimulator_cls(
        soundname=capture_dictionary["detection_sound"],
        lsl_streamer=lsl_streamer,
        sham=not capture_dictionary["stimulate"],
    )
    ...
    live_disp = (
        LiveDisplay(
            channel_names=capture_dictionary["signal_labels"],
            window_len=capture_dictionary["width_display"],
        )
        if capture_dictionary["display"]
        else Dummy()
    )
...
class Capture:
    def __init__(self, detector_cls=None, stimulator_cls=None):
        ...
        self.channel_states = ["bias"] + [
            "disabled" for _ in range(self.nb_channels - 1)
        ]
        ...
        if ADS:
            ...
            self.volume = self.mixer.getvolume()[
                0
            ]  # we will set the same volume on all channels
        else:
            self.mixer = DummyAlsaMixer()
        ...
        self.b_accordion_channels = widgets.Accordion(
            children=[
                widgets.GridBox(
                    [widgets.Label(f"CH{i+1}") for i in range(self.nb_channels)]
                    + self.chann_buttons,
                    layout=widgets.Layout(
                        grid_template_columns=f"repeat({self.nb_channels}, 90px)"
                    ),
                )
            ]
        )
        ...
        self.b_pause = widgets.ToggleButtons(
            ...
            button_style="",  # 'success', 'info', 'warning', 'danger' or ''
            tooltips=[
                "Detector and stimulator paused",
                "Detector and stimulator active",
            ],
        )
        ...
        self.b_power_line = widgets.ToggleButtons(
            ...
            disabled=False,
            button_style="",  # 'success', 'info', 'warning', 'danger' or ''
            tooltips=["North America 60 Hz", "Europe 50 Hz"],
        )
        ...
        self.b_signal_input = widgets.ToggleButtons(
            ...
            disabled=False,
            button_style="",  # 'success', 'info', 'warning', 'danger' or ''
            tooltips=["Read data from ADS.", "Read data from file."],
        )
        ...
        self.b_custom_fir = widgets.ToggleButtons(
            ...
            button_style="",  # 'success', 'info', 'warning', 'danger' or ''
            tooltips=[
                "Use the default 30Hz low-pass FIR from the Portiloop paper",
                "Use a custom FIR",
            ],
        )

        self.b_filename = widgets.Text(
            value="recording.edf", description="Recording:", disabled=False
        )

        self.b_frequency = widgets.IntText(
            value=self.frequency, description="Freq (Hz):", disabled=False
        )

        self.b_threshold = widgets.FloatText(
            value=self.threshold, description="Threshold:", disabled=True
        )

        self.b_polyak_mean = widgets.FloatText(
            value=self.polyak_mean, description="Polyak mean:", disabled=False
        )

        self.b_polyak_std = widgets.FloatText(
            value=self.polyak_std, description="Polyak std:", disabled=False
        )

        self.b_epsilon = widgets.FloatText(
            value=self.epsilon, description="Epsilon:", disabled=False
        )

        self.b_custom_fir_order = widgets.IntText(
            value=self.custom_fir_order, description="FIR order:", disabled=True
        )

        self.b_custom_fir_cutoff = widgets.IntText(
            value=self.custom_fir_cutoff, description="FIR cutoff:", disabled=True
        )
            ...
            children=[
                widgets.VBox(
                    [
                        ...
                        self.b_epsilon,
                        widgets.HBox(
                            [self.b_use_fir, self.b_use_notch, self.b_use_std]
                        ),
                    ]
                )
            ]
        ...
        self.b_duration = widgets.IntText(
            value=self.duration, description="Time (s):", disabled=False
        )

        self.b_filter = widgets.Checkbox(
            value=self.filter, description="Filter", disabled=False, indent=False
        )

        self.b_detect = widgets.Checkbox(
            value=self.detect, description="Detect", disabled=False, indent=False
        )

        self.b_stimulate = widgets.Checkbox(
            value=self.stimulate, description="Stimulate", disabled=True, indent=False
        )

        self.b_record = widgets.Checkbox(
            value=self.record, description="Record EDF", disabled=False, indent=False
        )

        self.b_lsl = widgets.Checkbox(
            value=self.lsl, description="Stream LSL", disabled=False, indent=False
        )

        self.b_display = widgets.Checkbox(
            value=self.display, description="Display", disabled=False, indent=False
        )
    ...
    def on_b_capture(self, value):
        ...
        if val == "Start":
            ...
            if (
                not self.python_clock
            ):  # ADS clock: force the frequency to an ADS-compatible frequency
