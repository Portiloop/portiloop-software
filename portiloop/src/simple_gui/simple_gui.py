...
class ExperimentState:
    ...
    def start(self):
        ...
        if self.sleep_timeout > 0:
            ...
            print(
                f"Currently: {self.time_started.timestamp()}, Pausing until: {self.time_unpause}"
            )
...
ui.html(
    f"Connected to: <strong>{portiloop_ID}</strong> (v{version} - {nb_channels} channels)"
)
ui.separator()
...
with ui.tab_panels(tabs, value=control_tab).classes("w-full"):
    ...
    with ui.tab_panel(control_tab).classes("w-full items-center"):
        ...
        with ui.column().classes("w-full items-center"):
            sd_card_checker = (
                ui.checkbox("SD Card").bind_value_from(exp_state, "sd_card").disable()
            )

            test_sound_button = ui.button("Test Sound 🔊", on_click=test_sound)

            stim_toggle = ui.toggle(
                ["Stim Off", "Stim On"],
                value="Stim Off",
            ...
            with ui.row():
                start_button = ui.button(
                    "Start ▶", on_click=start, color="green"
                ).classes("text-2xl")
                stop_button = ui.button("Stop", on_click=stop, color="orange").classes(
                    "text-2xl"
                )
            ...
            timer = ui.timer(
                ...
                lambda: time_label.set_text(
                    f'Timer: {str(datetime.now() - exp_state.time_started).split(".")[0]}'
                ),
            )
    ...
    with ui.tab_panel(output_tab).classes("w-full items-center"):
        ...
        with ui.column().classes("w-full items-center"):
            ...
            select_channel_display = ui.select(
                available_channels, value=available_channels[1], label="Display Channel"
            )
            select_channel_display.bind_value_to(exp_state, "selected_channel").classes(
                "w-1/2"
            )
    ...
    with ui.tab_panel(advanced_tab).classes("w-full items-center"):
        ...
        with ui.column().classes("w-full items-center"):
            ui.label(
                "If you are a subject in an experiment, do not change any of these options unless explicitly prompted to!"
            ).classes("text-1.5xl").style("color:#d9a011")
            ui.separator()
            space_label = (
                ui.label(f"Disk Usage: {psutil.disk_usage(os.getcwd())}%")
                .bind_text_from(exp_state, "disk_str")
                .classes("text-2xl")
            )
            possible_freqs = [50, 100, 250, 500, 1000]
            select_freq = (
                ui.select(possible_freqs, value=250, label="Sample Frequency (Hz)")
                .bind_value_to(exp_state, "select_freq")
                .classes("w-3/4")
            )
            ui.separator().classes("w-2/3")
            sleep_timeout = (
                ui.slider(min=0, max=40, value=0)
                .bind_value_to(exp_state, "sleep_timeout")
                .classes("w-3/4")
            )  # .props('label-always')
            ui.label().bind_text_from(
                sleep_timeout, "value", backward=lambda x: f"Sleep Timeout: {x} minutes"
            )
            ...
            start_button.bind_enabled_to(
                sleep_timeout_timer, "active", forward=lambda x: not x
            )
